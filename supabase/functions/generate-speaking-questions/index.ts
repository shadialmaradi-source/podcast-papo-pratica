...
serve(async (req) => {
  ...
  try {
    ...
    if (!topic || !level || !language) {
      return new Response(
        JSON.stringify({ error: "Missing topic, level, or language" }),
    ...
    const systemPrompt = `You are a language teaching assistant. Generate discussion questions for language learners.`;

    const userPrompt = `Generate 8 discussion questions about "${topic}" for a ${level} ${language} language learner.

Requirements:
- Questions 1-3: Easy (difficulty 1) - simple, personal questions
...
All questions should be in ${language} with natural phrasing appropriate for the ${level} level.`;
    ...
    console.error("generate-speaking-questions error:", e);
    return new Response(
